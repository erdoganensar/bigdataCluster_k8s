...
#echo $HADOOP_CLUSTER_DIR/apache-zookeeper-$ZOOKEEPER_VERSION.tar.gz
ZOOKEEPER_TARGET_FILE_NAME="$HADOOP_CLUSTER_DIR/apache-zookeeper-$ZOOKEEPER_VERSION-bin.tar.gz"

#spark variables
SPARK_VERSION=$(cat $BASE_DIR/spark-version)
SPARK_TARGET_FILE_NAME="$HADOOP_CLUSTER_DIR/spark-$SPARK_VERSION-bin-hadoop3.tgz"
...
if [ ! -x "$BASE_DIR/destory-cluster.sh" ]; then  
    chmod +x $BASE_DIR/destroy-cluster.sh
fi
...
BuildClusterImage(){
    ...
    sudo docker push $DOCKER_IMG_SVR/$1
...
if [ ! -f $ZOOKEEPER_TARGET_FILE_NAME ]; then
    ...
    wget -P $HADOOP_CLUSTER_DIR https://mirrors.tuna.tsinghua.edu.cn/apache/zookeeper/zookeeper-$ZOOKEEPER_VERSION/apache-zookeeper-$ZOOKEEPER_VERSION-bin.tar.gz
fi

#download spark
if [ ! -f $SPARK_TARGET_FILE_NAME ]; then
    echo -e "\033[44;37m downloading spark-$SPARK_VERSION.tgz \033[0m"
    wget -P $HADOOP_CLUSTER_DIR  https://archive.apache.org/dist/spark/spark-$SPARK_VERSION/spark-$SPARK_VERSION-bin-hadoop3.tgz
fi

#download hadoop 
if [ ! -f $HADOOP_TARGET_FILE ]; then
    echo -e "\033[44;37m downloading hadoop-$VERSION.tar.gz \033[0m"
    wget -P $HADOOP_CLUSTER_DIR https://archive.apache.org/dist/hadoop/core/hadoop-$VERSION/hadoop-$VERSION.tar.gz
fi

#download hive 
if [ ! -f $HIVE_TARGET_FILE_NAME ]; then
    echo -e "\033[44;37m downloading apache-hive-$HIVE_VERSION-bin.tar.gz \033[0m"
    wget -P $HADOOP_CLUSTER_DIR https://archive.apache.org/dist/hive/hive-$HIVE_VERSION/apache-hive-$HIVE_VERSION-bin.tar.gz
fi
...
if [ $is_pods_created -gt 0 ];then
    $BASE_DIR/destroy-cluster.sh $SLAVE_COUNT $POD_OWNER $CLUSTER_NAME $VERSION 
fi

#check whether base image  is running, if no then build it
is_base_img_created=`docker image list | grep ensaradaletai/bigdata_baseimg | wc -l`
if [ $is_base_img_created -eq 0 ];then
    cd  $BASE_DIR/baseimg
    docker build -t ensaradaletai/bigdata_baseimg .
    cd $BASE_DIR
fi
...
cp $BASE_DIR/k8s/template/postgresql-metadata-srv.yaml $K8S_PODS_DIR/postgresql-metadata-srv.yaml 
ReplacStrInFile "{cluster-metadb-config}" "$CLUSTER_NAME_PREFIX-$VERSION_NUMBER-metadb-config" $K8S_PODS_DIR/postgresql-metadata-srv.yaml 
ReplacStrInFile "{cluster_namespace}" $POD_OWNER $K8S_PODS_DIR/postgresql-metadata-srv.yaml 
db_port=$(( 5432 + $cluster_ports_offset ))
ReplacStrInFile "{cluster-metadb-listen-port}" $db_port $K8S_PODS_DIR/postgresql-metadata-srv.yaml 
ReplacStrInFile "{cluster_metadb_host_name}" "$CLUSTER_NAME_PREFIX-hadoop-$VERSION_NUMBER-metadb-srv" $K8S_PODS_DIR/postgresql-metadata-srv.yaml
ReplacStrInFile "{docker_img_srv_url}" $DOCKER_IMG_SVR  $K8S_PODS_DIR/postgresql-metadata-srv.yaml
ReplacStrInFile "{postgresql-root-pwd}" $HDFS_CLUSTER_ID $K8S_PODS_DIR/postgresql-metadata-srv.yaml
kubectl create -f $K8S_PODS_DIR/postgresql-metadata-srv.yaml
